            ...
            <?php
              if(isset($_POST['register_button'])){
                ...
                //make sure password and password-confirm match
                  ...
                  $query = mysql_query("SELECT 1 FROM users WHERE username='$username' LIMIT 1");
                  $run = mysql_fetch_array($query);
                  if (!empty($run[0])) {
                    echo '*Username already exists'; //failing for some reason
                  }
                  else {
                    if (!mysql_query("INSERT INTO users VALUES (NULL, '$name', '$username', '$email', '$password', 'crew', '$access_id')")) {  
                      echo mysql_error();
                    }
                    else {
                      ...
                      $run = mysql_fetch_array($query);
                      $username_test = $run[0];
                      echo "$username_test created successfully";
                    }
                  }
                }
              }
